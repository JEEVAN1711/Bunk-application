...
export const HistoricalLedgerPage: React.FC = () => {
  ...
  const exportToCSV = () => {
    ...
    if (activeTab === 'CLOSINGS') {
      csvContent += "Shift Number,Lead Cashier,Closed By Admin,Gross Sales (INR),Credit Given (INR),Payments Collected (INR),Expected Cash (INR),Actual Cash Handed (INR),Difference (INR),Status,Closed At\n";
      closings.forEach(c => {
        csvContent += `"${c.shiftNumber}","${c.cashierName}","${c.closedByAdminName}",${c.grossFuelSalesAmount},${c.creditGivenAmount},${c.creditPaymentsCollected},${c.expectedCashBalance},${c.actualCashInHand},${c.differenceAmount},"${c.closingStatus}","${c.closedAt}"\n`;
      });
  ...
  const handleSendWhatsAppForShift = (c: DutyClosing) => {
    ...
    if (shiftPumps.length > 0) {
      ...
      const totalL = shiftPumps.reduce((s, r) => s + (r.totalLiters || 0), 0);
      text += `   ↳ *Total Fuel Dispensed: ${totalL.toFixed(2)} Liters*\n\n`;
    }
  ...
  return (
    <div className="space-y-6 pb-12">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white/95 p-6 rounded-3xl border border-slate-200 shadow-md">
        <div>
