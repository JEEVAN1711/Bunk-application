...
import { CreditEntry, ExpenseEntry } from '../types';
...
export const CashierPortal: React.FC<{ onNavigate?: (page: string) => void }> = ({ onNavigate }) => {
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="glass-panel rounded-3xl p-6 border border-slate-200 shadow-md bg-white/95 relative overflow-hidden">
        ...
        {!activeDuty ? (
          /* Standby Alert when no shift is running */
            ...
            <div className="flex items-start sm:items-center gap-3.5">
              ...
              <div>
                <h3 className="font-black text-sm sm:text-base text-amber-950">
                  Shift Standby - {isAdmin ? 'No Active Shift Running' : 'Waiting for Admin to Start Shift'}
                </h3>
                <p className="text-xs text-amber-800 mt-0.5 max-w-xl">
                  {isAdmin
                    ? 'Assign staff and launch a new duty shift to initialize pump meters, unlock cashier POS, and record credit slips.'
                    : 'Credit slip entry and daily expenses are locked until the Station Admin opens a new duty shift. Once started, this screen will unlock automatically in real-time.'}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2 self-start sm:self-auto">
              {isAdmin ? (
                <button
                  ...
                  className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white text-xs font-black transition-all shadow-md flex items-center gap-1.5 cursor-pointer active:scale-95"
                >
          ...
          /* Shift is ACTIVE and current user is ADMIN */
          <div className="mt-6 space-y-3">
            <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-sm">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-emerald-100 text-emerald-700 flex items-center justify-center border border-emerald-300 flex-shrink-0">
                  <CheckCircle2 className="w-5 h-5" />
                </div>
                <div>
                  <h4 className="font-black text-xs sm:text-sm text-slate-900">
                    Active Duty Shift: <span className="font-mono-numbers text-emerald-800">{activeDuty.shiftNumber}</span>
                  </h4>
                  <p className="text-xs text-slate-600">
                    Lead Cashier Assigned: <strong className="text-blue-700 font-bold">{activeDuty.cashierName}</strong>
                    {activeDuty.supportCashierName ? ` | Support: ${activeDuty.supportCashierName}` : ''}. Station Admin operational oversight enabled.
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <button
                  ...
                  className="px-3 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-black text-xs flex items-center gap-1.5 shadow-sm"
                >
            ...
            {/* Admin Quick Action Touch Buttons */}
              ...
              >
                <div className="flex items-center gap-3.5">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-sky-100 font-normal">Diesel / Petrol / Oil Fuel Entry (Shift {activeDuty.shiftNumber})</div>
                  </div>
                </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        {/* Card 3: Cashier Role Scope */}
        <div className="glass-panel rounded-3xl p-5 border border-indigo-200 bg-white/95 shadow-sm space-y-1">
          <div className="flex items-center justify-between text-xs font-bold text-indigo-700">
            <span>Cashier Duty Scope</span>
            <ShieldCheck className="w-4 h-4 text-indigo-600" />
          </div>
          <p className="text-sm font-black text-indigo-950 mt-1">
            Fuel Credit Slips Only
          </p>
          <p className="text-[11px] text-slate-500 font-medium">
            Customer repayments are Admin-managed &bull; Cashier shift cash is protected
          </p>
        </div>
      </div>
      ...
      <div className="glass-panel rounded-3xl p-6 border border-slate-200 bg-white/95 shadow-sm space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-slate-500 font-mono-numbers font-bold">
            {shiftCredits.length + shiftExpenses.length} shift entries
          </span>
        </div>

        <div className="space-y-2.5">
          {shiftCredits.length === 0 && shiftExpenses.length === 0 ? (
            <p className="text-xs text-slate-500 text-center py-8">
              No credit or expense entries recorded in this shift yet. Use the buttons above to record.
            </p>
          ) : (
            <>
