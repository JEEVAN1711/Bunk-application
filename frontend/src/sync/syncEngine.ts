...
class SyncEngine {
  ...
  /**
   * Enqueue and directly sync multiple entities in a single atomic batch payload
   */
      ...
      try {
        ...
        if (response.ok && contentType.includes('application/json')) {
          for (const item of items) {
            await this.markEntitySynced(item.entityType, item.syncId);
          }
          this.currentStatus = 'ONLINE';
          this.notifyListeners();
          window.dispatchEvent(new CustomEvent('bunk_cloud_synced'));
        ...
        console.warn('Direct batch write failed, queuing locally for offline sync:', err);
      }
    }

    // Fallback: Queue all items locally
  ...
  public async syncAllLocalDataToCloud(): Promise<{ success: boolean; totalItems: number; error?: string }> {
    try {
      // 1. FIRST flush all pending items in syncQueue (such as pending DELETE operations)
      await this.triggerSync();
    ...
    try {
      ...
      await db.transaction('rw', [
        ...
        if (Array.isArray(cloudData.users)) {
          const cloudUserIds = new Set(cloudData.users.map((u: any) => u.id).filter(Boolean));
          const allUserQueue = await db.syncQueue.where('entityType').equals('USER').toArray();
          const pendingUserIds = new Set(allUserQueue.map(q => q.syncId));
          const pendingDeleteUserIds = new Set(
            allUserQueue.filter(q => q.action === 'DELETE').map(q => q.syncId)
          );

          // Prune any deleted or orphaned users from local cache
          const localUsers = await db.users.toArray();
          for (const lu of localUsers) {
            if (!cloudUserIds.has(lu.id) && !pendingUserIds.has(lu.id)) {
              await db.users.delete(lu.id);
            }
            if (pendingDeleteUserIds.has(lu.id)) {
              await db.users.delete(lu.id);
            }
          }

          for (const u of cloudData.users) {
            // NEVER re-insert a user that has been deleted and pending delete in sync queue!
            if (pendingDeleteUserIds.has(u.id)) {
              continue;
            }
            await db.users.put({
              id: u.id,
        ...
        if (Array.isArray(cloudData.customers)) {
          ...
          const allCustQueue = await db.syncQueue.where('entityType').equals('CUSTOMER').toArray();
          const pendingCustomerIds = new Set(allCustQueue.map(q => q.syncId));
          const pendingDeleteCustIds = new Set(
            allCustQueue.filter(q => q.action === 'DELETE').map(q => q.syncId)
          );

          // Prune any deleted customers from local cache
          const localCustomers = await db.customers.toArray();
          for (const lc of localCustomers) {
            if (!cloudCustomerIds.has(lc.id) && !pendingCustomerIds.has(lc.id) && lc.synced) {
              await db.customers.delete(lc.id);
            }
            if (pendingDeleteCustIds.has(lc.id)) {
              await db.customers.delete(lc.id);
            }
          }

          for (const c of cloudData.customers) {
            // NEVER re-insert a customer that has been deleted and pending delete in sync queue!
            if (pendingDeleteCustIds.has(c.id)) {
              continue;
            }
        ...
        if (Array.isArray(cloudData.dutyShifts)) {
          ...
          const pendingDutyIds = new Set(
            (await db.syncQueue.where('entityType').equals('DUTY').toArray()).map(q => q.syncId)
          );
          ...
          for (const ls of localShifts) {
            ...
            if (!cloudShift && !pendingDutyIds.has(ls.id) && ls.synced) {
              // Delete orphaned local shift that does not exist in cloud database AND was previously synced
              ...
              } else if (!isClosed && cloudShift.status === 'ACTIVE') {
                await db.dutyShifts.update(ls.id, {
                  status: 'ACTIVE',
                  endTime: undefined
                });
          ...
          for (const s of cloudData.dutyShifts) {
            ...
            await db.dutyShifts.put({
              ...
              endTime: isClosed ? s.endTime : undefined,
        ...
        if (Array.isArray(cloudData.creditEntries)) {
          const cloudCreditIds = new Set(cloudData.creditEntries.map((cr: any) => cr.id).filter(Boolean));
          const pendingCreditIds = new Set(
            (await db.syncQueue.where('entityType').equals('CREDIT').toArray()).map(q => q.syncId)
          );
          const localCredits = await db.creditEntries.toArray();
          for (const lc of localCredits) {
            if (!cloudCreditIds.has(lc.id) && !pendingCreditIds.has(lc.id)) {
              await db.creditEntries.delete(lc.id);
            }
          }
        ...
        if (Array.isArray(cloudData.paymentEntries)) {
          const cloudPaymentIds = new Set(cloudData.paymentEntries.map((p: any) => p.id).filter(Boolean));
          const pendingPaymentIds = new Set(
            (await db.syncQueue.where('entityType').equals('PAYMENT').toArray()).map(q => q.syncId)
          );
          const localPayments = await db.paymentEntries.toArray();
          for (const lp of localPayments) {
            if (!cloudPaymentIds.has(lp.id) && !pendingPaymentIds.has(lp.id)) {
              await db.paymentEntries.delete(lp.id);
            }
          }
        ...
        if (Array.isArray(cloudData.dutyClosings)) {
          const cloudClosingIds = new Set(cloudData.dutyClosings.map((cl: any) => cl.id).filter(Boolean));
          const pendingClosingIds = new Set(
            (await db.syncQueue.where('entityType').equals('DUTY_CLOSING').toArray()).map(q => q.syncId)
          );
          const localClosings = await db.dutyClosings.toArray();
          for (const lcl of localClosings) {
            if (!cloudClosingIds.has(lcl.id) && !pendingClosingIds.has(lcl.id)) {
              await db.dutyClosings.delete(lcl.id);
            }
          }
        ...
        if (Array.isArray(cloudData.expenseEntries)) {
          const cloudExpenseIds = new Set(cloudData.expenseEntries.map((exp: any) => exp.id).filter(Boolean));
          const pendingExpenseIds = new Set(
            (await db.syncQueue.where('entityType').equals('EXPENSE').toArray()).map(q => q.syncId)
          );
          const localExpenses = await db.expenseEntries.toArray();
          for (const le of localExpenses) {
            if (!cloudExpenseIds.has(le.id) && !pendingExpenseIds.has(le.id)) {
              await db.expenseEntries.delete(le.id);
            }
          }
  ...
  /**
   * Complete Fresh Start: Permanently purge ALL data including ALL users, shifts, meters, credits, payments, and customers from BOTH local Dexie and Cloud MongoDB.
   */
  public async resetEntireDatabaseIncludingUsers(): Promise<void> {
    try {
      // 1. Clear local IndexedDB tables including users and syncQueue
      await Promise.all([
        db.users.clear(),
        db.customers.clear(),
        db.dutyShifts.clear(),
      ...
      // 2. Call backend cloud full wipe endpoints
      await Promise.allSettled([
        fetch(`${getApiBaseUrl()}/api/sync/database/reset-all`, { method: 'POST' }),
        fetch(`${getApiBaseUrl()}/api/sync/database/reset-all`, { method: 'DELETE' })
      ]);

      window.dispatchEvent(new CustomEvent('bunk_cloud_synced'));
    } catch (err) {
      console.error('Failed to execute complete database wipe:', err);
    }
  }
}
