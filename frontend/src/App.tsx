...
const AppContent: React.FC = () => {
  ...
  const renderPage = () => {
    ...
    switch (currentPage) {
      ...
      case 'closing':
        return role === 'ADMIN' ? <DutyClosingPage onNavigate={handleNavigate} /> : <CashierPortal />;
      case 'customer':
        return <CustomerPortal />;
      case 'staff':
        return role === 'ADMIN' ? <StaffManagementPage /> : <CashierPortal />;
      case 'history':
        return <HistoricalLedgerPage />;
      case 'settings':
        return role === 'ADMIN' ? <SettingsPage /> : <CashierPortal />;
      default:
        return role === 'ADMIN' ? <AdminDashboard onNavigate={handleNavigate} /> : <CashierPortal />;
    }
  };
