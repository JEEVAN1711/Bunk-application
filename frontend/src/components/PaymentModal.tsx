...
export const PaymentModal: React.FC<PaymentModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Customer debt settlement is strictly maintained in the Customer Ledger and does not attach to cashier shift
      const newPayment = await recordPayment({
        customerId: selectedCustomerId,
        customerName: selectedCustomer?.name || 'Customer',
        dutyId: 'station-ledger-settlement',
      ...
      <div className="w-full max-w-lg bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-black text-slate-900 text-base">Customer Account Settlement (Passbook)</h3>
              <p className="text-xs text-slate-500 font-medium">Admin Debt Clearance &bull; Does not deduct or alter cashier shift cash</p>
            </div>
          </div>
          <button
            onClick={() => {
