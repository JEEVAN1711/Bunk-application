...
export const NewDutyModal: React.FC<NewDutyModalProps> = ({ isOpen, onClose }) => {
  ...
  const generateUniqueShiftNumber = async (dateStr: string, slot: string) => {
    const base = `SHIFT-${dateStr}-${slot}`;
    const allShifts = await db.dutyShifts.toArray();
    const matching = allShifts.filter(s => s.shiftNumber === base || s.shiftNumber.startsWith(base + '-'));
    if (matching.length === 0) return base;

    let maxSuffix = 1;
    for (const s of matching) {
      if (s.shiftNumber === base) {
        if (maxSuffix < 1) maxSuffix = 1;
      } else {
        const parts = s.shiftNumber.split('-');
        const last = parseInt(parts[parts.length - 1], 10);
        if (!isNaN(last) && last > maxSuffix) {
          maxSuffix = last;
        }
      }
    }
  ...
  const resetToCurrentDateTime = async () => {
    const now = new Date();
    ...
    const uniqueShift = await generateUniqueShiftNumber(dateStr, slot);
    setShiftNumber(uniqueShift);
  };
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const loadUsers = async () => {
        let users = await db.users.filter(u => u.active).toArray();
        if (users.length === 0) {
          await syncEngine.pullAndHydrateFromCloud();
          users = await db.users.filter(u => u.active).toArray();
        }
        ...
        if (cashs.length > 0 && !selectedCashierId) {
          setSelectedCashierId(cashs[0].id);
        }
      };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Ensure unique shift number if a shift was already closed with this name earlier today
      const baseShift = shiftNumber.trim();
      const allShifts = await db.dutyShifts.toArray();
      const matching = allShifts.filter(s => s.shiftNumber === baseShift || s.shiftNumber.startsWith(baseShift + '-'));
      let finalShiftNumber = baseShift;
      if (matching.length > 0) {
        let maxSuffix = 1;
        for (const s of matching) {
          if (s.shiftNumber === baseShift) {
            if (maxSuffix < 1) maxSuffix = 1;
          } else {
            const parts = s.shiftNumber.split('-');
            const last = parseInt(parts[parts.length - 1], 10);
        ...
        finalShiftNumber = `${baseShift}-${maxSuffix + 1}`;
      }
      ...
      console.error('Failed to start duty shift:', err);
      alert('Failed to launch duty shift: ' + (err.message || 'Unknown error'));
    } finally {
