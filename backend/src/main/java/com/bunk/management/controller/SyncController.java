...
public class SyncController {
    ...
    @PostMapping("/database/reset-all")
    public ResponseEntity<Map<String, Object>> resetEntireDatabase() {
        syncService.resetEntireDatabase();
        return ResponseEntity.ok(Map.of("success", true, "message", "Entire database wiped, including all users. Ready for fresh manual setup."));
    }

    @DeleteMapping("/database/reset-all")
    public ResponseEntity<Map<String, Object>> deleteEntireDatabase() {
        syncService.resetEntireDatabase();
        return ResponseEntity.ok(Map.of("success", true, "message", "Entire database wiped, including all users. Ready for fresh manual setup."));
    }
}
